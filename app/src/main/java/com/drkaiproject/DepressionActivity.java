...
public class DepressionActivity extends AppCompatActivity {
        ...
        final JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.POST, url, jsonObject, new Response.Listener<org.json.JSONObject>() {
            ...
            public void onResponse(JSONObject response) {
                try {
                    ...
                    Log.d("sendData_response", res.toString());
        ...
        final JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.POST, url,jsonObject, new Response.Listener<org.json.JSONObject>() {
            ...
            public void onResponse(JSONObject response) {
                try {
                    ...
                    Log.d("sendMachine_response", res.toString());
                    if (!res.getString("result").equals("1")) throw new Exception();
                    else{
                        Intent intent = new Intent(DepressionActivity.this, MainActivity.class);
    ...
    public synchronized void btnMethod(View view) {
        ...
        try{
            jsonObject.put("depSum", Integer.toString(depSum));
            jsonObject.put("check", "1");
            Intent intent = new Intent(DepressionActivity.this, HealthServeyActivity.class);
            intent.putExtra("account", jsonObject.toString());
            startActivity(intent);
        } catch (JSONException e) {
            e.printStackTrace();
        ...
        /* original code
        try {
            if (radio1.isChecked()) {//예
                jsonObject.put("depSum", Integer.toString(depSum));
                jsonObject.put("check", "1");
                Intent intent = new Intent(DepressionActivity.this, HealthServeyActivity.class);
                intent.putExtra("account", jsonObject.toString());
                startActivity(intent);
            } else if (radio2.isChecked()) {//아니요
                jsonObject.put("depSum", Integer.toString(depSum));
                jsonObject.put("check", "0");
                //Toast.makeText(getApplicationContext(), jsonObject.toString(), Toast.LENGTH_LONG).show();
                Log.d("DepressionAcitivity", jsonObject.toString());
                try {
                    sendData();
                } catch (Exception e) {
                    e.printStackTrace();
                ...
                //Toast.makeText(DepressionActivity.this, "건강검진 여부를 선택해주세요", Toast.LENGTH_SHORT).show();
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
    ...
    public void onBackPressed() {
        super.onBackPressed();
        Intent intent = new Intent(DepressionActivity.this, ServeyActivity.class);
        startActivity(intent);
    }
}
