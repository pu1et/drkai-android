...
public class UserInfoChange2 extends AppCompatActivity {
    ...
    //나이, 결핍지수, 당뇨병 가족력, 고혈압, 뇌졸중, 당뇨병, 천식, BMI
    String age, carstairs_level, history_diabetes, is_hypertension, is_previousStroke, is_diabetes, prior_asthma, bmi, history_coronaryArtery;
    // history_diabetes == HE_DMfh, history_coronaryArtery == HE_IHDfh, is_diabetes == DE1_dg, is_hypertension == DI1_dg, is_previousStroke == DI3_dg, bmi == bmi, age == age, gender == gender, is_obesity => 새로 생성해야됨(bmi기준), alcohol(새로생성해야됨)-이전데이터기준으로, smoking(새로생성해야됨)-이전데이터기준으로, PhA(새로생성해야됨)-이전데이터기준으로, area_num == area;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            history_cancer = jsonObject.getString("history_cancer");
            depression_risk = jsonObject.getString("depSum");
            ...
            else if(Integer.parseInt(smoking) >= 5 && Integer.parseInt(smoking) < 10) smoking_gastric = "3";
            else if(Integer.parseInt(smoking) >= 10) smoking_gastric = "4";
        ...
        go_main.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (textToString()) {
                    ...
                    //임의의 데이터
                    check_user(); // check_user -> sendData -> storeDisease -> MainActivity
                }
            }
        });
    }
            ...
            public void onResponse(JSONObject response) {
                try {
                    ...
                    if (!res.getString("result").equals("1")) throw new Exception();
                        ...
                        Intent intent = new Intent(UserInfoChange2.this, MainActivity.class);
    ...
    public void radioToString() {
        ...
        //"0","1"로 바꾸기
        String[] str = {is_atrialFibrillation, DLD_serve, is_hypercholesterolemia, is_chemicHeartDisease, meal_reg, salt_pref};
        for (int i = 0; i < 6; i++) {
